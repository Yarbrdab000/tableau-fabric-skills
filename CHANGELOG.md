...
- **tableau-migration (skill `2.56.0` -> `2.57.0`): recognise the NUMERIC spelling of a
  ...
  range degenerated to a single value (`<min>2</min><max>2</max>` -- "show this sheet when the
  parameter equals 2"). Only the categorical form was recognised, so every numeric swap fell
  through to the `aggregate/measure filter ... is not mapped` warning and its worksheet lost the
  control. A range is treated as a swap pin only when it is CLOSED and DEGENERATE (both bounds
  present and equal); a genuine spanning or half-open range on the same passthrough calc is still
  a real data filter. Measured on a real 164-visual workbook: 16 false measure-filter warnings
  retired, 4 KPI visuals promoted from `warned` to a clean `rebuilt`, and 2 honest
  dashboard-level swap disclosures surfaced. Emitted PBIR output is byte-identical across all 164
  visuals -- this corrects the REPORT, and is the precondition for ever collapsing a swap pair.
- **tableau-migration (skill `2.55.0` -> `2.56.0`):** resolve a parameter-driven boolean filter inside its OWN datasource island. On a consolidated multi-datasource workbook the same physical table is emitted once per island (`pmdm__ProgramEngagement__c` + `... (Intake)` + `... (Assessments)` ...), so a bare field reference is AMBIGUOUS against the pooled resolver. `_param_predicate_flags` was handed that pooled resolver -- while the measure path already resolves each calc inside its own island -- so the predicate stubbed with "unresolved/ambiguous field", the calc won no keep-flag, and every worksheet that filtered on it silently lost its filter. On a real Salesforce case-management workbook ONE calc (`Date FIlter`) accounted for 29 identical dropped filters; it now resolves to the same table the adjudicated reference chose, and the visuals it scopes went from 14 to 40. Corpus-wide the "aggregate/measure filter not mapped" class fell 59 -> 30 with no other workbook touched. Separately, the row-predicate wrapper measure is no longer named with an opaque content hash: several visual types label a field with the MODEL MEASURE NAME rather than the projection's `nativeQueryRef`, so `Number of Clients (filtered ed813d64)` was rendering on the face of the report. The measure now carries the short suffix `(filtered)` and the full provenance (which Tableau filters narrowed it) rides its TMDL annotation, where an author can read it without it intruding on the page.
- **tableau-migration (skill `2.54.0` -> `2.55.0`):** translate the table calcs that were failing at the FIRST head rather than on their substance. A table-calc head is an expression PRIMARY, so a composite like `INDEX()/SIZE()` or `WINDOW_SUM(..) - WINDOW_SUM(..) * ..` previously died on the leading head and stubbed the whole calc; addressing is now computed through lazy per-family closures so every historical fallback string stays byte-identical. Four supporting fixes: an AGGREGATED pill on Rows is the mark's measure, not an addressing dimension (Tableau partitions only by dimensions), so it is skipped without dropping a real dimension beside it; value-preserving heads (MINX/MAXX-backed, LOOKUP, TOTAL) propagate their inner result type, so `ATTR([Order Date]) = WINDOW_MAX(MAX([Order Date]))` stops failing as "incomparable types"; a pill whose own formula provably aggregates is treated as a measure, while row-level AND LOD formulas stay fail-closed (an LOD is routinely used AS a dimension); and a named-calc reference such as `Total([Referrals])` inlines the referent's formula so the seam sees an aggregate, failing closed when the reference is itself a table calc. Columns-scope addressing is covered for the order-INSENSITIVE cases only -- an order-sensitive Columns calc still hands off rather than assume a direction. Measured over 29 workbooks: 6 more calcs translate, 5 fewer stubs, 5 fewer needing review, and NO workbook lost a translation. The two headline translations were render-verified in Power BI Desktop, since both emit window functions (`ROWNUMBER`/`WINDOW`/`ORDERBY`) whose same-table rules the engine rejects at load if violated.
- **tableau-migration (skill `2.53.0` -> `2.54.0`):** make a rebuilt date axis actually filter its fact. Three defects compounded into the same silent failure -- a time series that renders as a flat line or a solid block because every bucket returns the grand total. (1) The primary-date selector only recognised a column named literally `Date` or `*order*date*`, so a fact whose dates are `CreatedDate` + `ClosedDate` matched neither, went "ambiguous", and had EVERY date relationship emitted inactive -- the calendar could not filter it at all. The record-creation date is now an explicit primary-date convention (the ambiguity guard is unchanged: two competing conventions still decline). (2) The report binder matched active date keys by COLUMN NAME ALONE, so a fact with no active relationship still had its axis rebound onto the calendar whenever some other table's active date shared the name (`CreatedDate` is near-universal). Contested names are now identified by the model build and declined by the binder -- expressed on the name rather than on (table, column) because at bind time the field still carries the WORKBOOK's relation name (a Tableau extract emits `Orders_<hex32>`), not the model's table name. (3) The generated calendar used `CALENDARAUTO()`, which scans every dateTime column in the model -- one contact birthdate dragged a 2017-2021 workbook's calendar back to 1941 and buried the business range under tens of thousands of empty rows. It now spans only the fact date columns the calendar actually relates to, rounded out to whole years so Mark-as-Date's contiguity still holds. Verified by live DAX: months that previously all returned the same grand total now return the correct per-month values.
- **tableau-migration (skill `2.52.0` -> `2.53.0`):** open a rebuilt parameter-control slicer on the value the author saved. A Tableau parameter control always shows its current value; a Power BI slicer with no selection shows "All". Aliased list parameters already carried their selection, but a SCALAR parameter -- a date picker, a numeric what-if -- had no aliased members and its display IS its value, so it declined and rendered "All" while the DAX behind it quietly used the authored default. The face of the report disagreed with its own numbers. Scalar controls now emit an exact-value preselection (a semantic-query `datetime'...'` / numeric literal compared against the resolved picker column). The gate is generic -- parameter datatype + saved literal + model-resolved picker column, never a caption -- so it covers the class rather than one workbook, and a value that cannot be parsed into a typed literal still declines rather than guessing. Also accepts a plain string-list parameter whose stored value already IS the member label.
- **tableau-migration (skill `2.51.0` -> `2.52.0`):** apply a Tableau boolean calc filter as the ROW-level keep it actually is. A filter pinned to a calculated field with `member="true"` -- the standard way a workbook applies a parameter-driven date window -- was rebuilt as a visual-level filter on a synthesized keep-flag MEASURE (`[Flag] = 1`). A measure filter is evaluated at the visual's GROUP grain, so on an ungrouped visual (every KPI card) the "group" is the whole table and the flag is vacuously true: the card silently showed unfiltered totals while the source workbook showed a windowed number. The model build now records the row predicate beside each flag, and the affected visuals are rebound to `CALCULATE(<original projection>, FILTER('<fact>', <predicate>))` measures with the superseded flag filter removed from those visuals only. Fail-closed: a visual is rewritten ONLY when every measure/aggregation projection on it can be wrapped, so a shape we cannot express faithfully keeps its previous behaviour rather than being half-rewritten. Measured on a real Salesforce case-management workbook, the Intake KPIs moved from 1354 / 1100 / 252 to 820 / 742 / 81 -- matching the source exactly -- while the two KPIs the workbook does not date-scope stayed at 548 / 20.
- **tableau-migration (skill `2.50.0` -> `2.51.0`):** carry an applied Tableau filter SELECTION into the PBIR slots that actually restrict data. A slicer's open-on selection is `visual.objects.general[].properties.filter.filter`, not `filterConfig` -- the latter only limits which members a slicer OFFERS and never filters the page, so an authored selection previously left the slicer reading "All" while every other visual showed unfiltered numbers. The two are now alternatives, never partners, so a preselected slicer offers its full member list with the authored members ticked. Field-parameter controls select through their hidden group-by column (the only variant that opens on the authored value). Separately, a worksheet filter with no quick-filter card is a fixed property of that worksheet and now becomes a visual-level `filterConfig`; previously only SURFACED filters were rebuilt, so unsurfaced restrictions were dropped and those visuals silently widened to the whole table. Selections whose member has no type-safe literal against the rebuilt column (Tableau serialises a boolean member as the string "true") are declined with a fidelity warning rather than emitted, because a mistyped literal on a chart is a hard error tile. Exclude mode continues to decline. Measured over 29 workbooks: 26 slicers gained a selection, 6 visuals gained an applied filter, 0 structural changes.
...
- **tableau-migration (skill `1.63.0` → `1.64.0`): Emit measure `formatString` decoded from the Tableau
  ...
  `0.13` instead of `12.6%`, and currency measures losing their `$`). Additive and conservative — a calc
  with no decodable format is byte-identical to before.**
  - **Conservative measure decoder.** New `tmdl_generate.tableau_measure_format_to_pbi` decodes ONLY the
    explicit lowercase author prefixes (`c`/`n`/`p`/`*`, e.g. `p0.0%` → `0.0%`, `c"$"#,##0` → currency) and
    deliberately **declines the ambiguous built-in `C<lcid>%` form** — in the wild that code decorates both
    currency and percent calcs, so decoding it would risk mis-rendering a dollar figure as a percentage.
    Undecodable/empty codes yield `None` (no format), preserving the type-derived floor.
  - **Additive serializer param.** `generate_measure_tmdl(format_string=...)` emits a `formatString:` line
    between the measure expression and its `lineageTag`; omitting it (or `None`) is byte-identical to the
    prior output.
  - **Both calc extractors stamp the format.** `migrate_estate.extract_calculations` (`.twb`) and
    `connection_to_m.extract_calcs` (`.tds`) now attach `format_string` to a measure calc when its
    `default-format` decodes; `assemble_model._measures_part` threads it into every `generate_measure_tmdl`
    call site.
  - **Coverage.** New `tests/test_measure_format_wiring.py` (10 tests): decoder accepts explicit codes /
    declines `C1033%` / `None` on garbage; the serializer param emits + stays byte-identical without it;
    both extractors stamp the right calc and skip the ambiguous/absent ones; end-to-end through
    `_measures_part`. The physical-column path (`test_default_format_wiring.py`) is untouched.
