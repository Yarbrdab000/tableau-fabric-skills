...
- **`tableau-migration` (skill `2.366.0` → `2.367.0`): several date PARTS of one date sharing an
  ...
  **The axis LABELS change, and that is a real fidelity delta.** Tableau draws a two-tier discrete
  header (`2013` over `January`); the folded axis renders one tier. Verified at the render on all
  three visual types rather than inferred, because the obvious inference is wrong: the shared
  `Month Start` column carries `formatString: Short Date`, which would predict `1/1/2013`, and that
  is **not** what the axis shows.
  ...
  The mechanism: no `axisType` is emitted, so Power BI defaults a `dateTime` Category to a
  **continuous date axis**, which labels by its own range- and width-aware granularity and does not
  consult the column's format string. So the rendered result already lands close to the source
  (`Jan 2013` against Tableau's `2013 / January`) and needs no visual-level format override — but it
  is a property of the *axis*, not of the column, and reading `Short Date` off the TMDL will mislead
  anyone who tries to predict it.

  **Do not "fix" this by changing `Month Start`'s `formatString`**: 17 emitted models carry that
  column and the continuous-truncation path binds it too, so it is far wider than this release.
...
- **`tableau-migration` (skill `2.323.0` → `2.325.0`): an overlapping-bar rebuild now carries
  ...
  emit site indexed `dataPoint` unconditionally.
