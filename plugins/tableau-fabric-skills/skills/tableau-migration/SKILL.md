...
| [`scripts/build_diff.py`](scripts/build_diff.py) | **Compare two migration output trees and report what SUBSTANTIVELY changed** — after an engine upgrade, when reproducing a defect, or to confirm a change did only what it claimed. Classifies the terms that differ between any two runs and carry no information (the build root embedded in each model's M `Source = ...`, in plain / POSIX / **JSON-escaped** spellings; `lineageTag` identity GUIDs; timestamps) and reports each **separately** rather than masking it — so `0 GUID differences` stays distinguishable from `GUIDs were masked`. CLI (`build_diff.py <tree-a> <tree-b> [--show N] [--strict]`) **and** importable (`compare`, `normalize`, `root_spellings`). Exit `0` clean / `1` substantive / `2` instrument error, so it is usable as a gate and a wrong path can never read as a clean run; `compare()` raises rather than returning a clean result on an empty tree. |
...
> **⛔ Completion gate — read `report["pending_gates"]` before you say "done."** Every run now emits a
> first-class **`pending_gates`** list on `report.json` (and a loud **`⏳ PENDING REVIEW GATES`** banner
> at the top of `summary.md`) enumerating the user-gated offers the deterministic pass still owes — the
> **second compiler** for any stubbed calc (`summary.needs_review_total > 0`, step 3) and the **Tier-3
> dashboard audit** for any warned visual (`summary.visuals_warned > 0`, step 5). **While that list is
> non-empty, the migration is NOT complete: you must present and *offer* each gate first.** The user may
> decline any offer — that ships the deterministic result as-is, a complete and honest outcome — but the
> offer is owed. Do not narrate "✅ Migration Complete" while `pending_gates` is non-empty and unoffered.
...
3. **Offer the second compiler on every stubbed calc — an explicit, user-gated opt-in.** _Guard first: this step applies **only** when `report["summary"]["needs_review_total"] > 0`. If it is `0`, nothing is stubbed — there is nothing to offer; move on without re-reading the report._ When the deterministic pass leaves any calc stubbed (`report["summary"]["needs_review_total"] > 0`, also in `summary.md`'s **Next step** section and each `report["datasources"][n]["translation_handoff"]`), **present the stubbed calcs and ask the user whether to run the LLM-assisted second compiler — do not proceed on your own.** You must always *offer* it when there are stubs (never silently ship as if nothing can be done), but you *run* it **only** on an explicit yes/`GO`. If the user declines, the deterministic result ships as-is and every stub keeps its preserved `TableauFormula` — a complete, honest outcome (a stubbed calc is not a failed migration). **Once the user replies `GO`, run the full pass and auto-land every validated candidate (no per-calc approval):** work the Tier-1 loop per [second-compiler.md](resources/second-compiler.md): author the leanest *faithful* candidate DAX → `check_candidate_dax` (syntactic gate) → reconcile against the oracle when data is landed → **land every validated candidate automatically** via `approved_calc_dax` → redeploy. The **faithful-or-stub** charter still binds at the *landing* step: once authorized, the pass runs in full, but a calc with no faithful DAX form stays an inert stub (original `TableauFormula` preserved) — the validation gate, not the user prompt, is what prevents a guess going live. **You ARE the second compiler — there is no script that authors the DAX for you; you write it, the gate validates it, and `--approved-dax` (estate CLI) or `approved_calc_dax=` (in-process) lands it.** See [second-compiler.md](resources/second-compiler.md) §"Read this first" and §"The estate-CLI loop, exactly" for the verbatim author→gate→`approved_dax.json`→re-run sequence and the exact JSON shape.
   ...
   > **Author ROOTS first, and read `blocked_by` before you read `fallback_reason`.** A calc that fell back only because a calc it *references* is unmigrated reports the **dependency's** error as its own — so its `fallback_reason` describes a different calc, and authoring against it means solving a problem that calc does not have. Each entry carries `blocked_by` (the referenced calcs that are themselves needs-review); work only the entries whose `blocked_by` is empty, then re-run — the native `measure_refs` cascade usually clears the dependents for free, so `needs_review_total` drops by **more** than the count you landed. Note also that `category_guidance` / `fields` / `formula` ship on `translation_handoff.requests` only, **not** on the concise `needs_review` list. See [second-compiler.md](resources/second-compiler.md) §"Two ways this manifest will mislead you".
...
5. **Offer the dashboard audit (Tier 3) on every warned visual — an explicit, user-gated opt-in, the same procedure as the second compiler.** _Guard first: this step applies **only** when the deterministic viz rebuild flagged something — a workbook with a `viz_fidelity` entry whose `status == "warned"`, or a per-workbook remediation worklist with `summary.visuals_flagged > 0` (equivalently the audit bundle's `summary.needs_attention > 0`). If nothing is warned/flagged, there is nothing to adjudicate; move on._ When there are warned visuals, **present them and ask whether to run the LLM-assisted dashboard audit — do not proceed on your own.** Always *offer* when visuals are warned; *run* it **only** on an explicit `GO`. If the user declines, the deterministic rebuilt reports ship as-is (every faithfully-bound visual intact) — a complete, honest outcome. **Once the user replies `GO`, run the full pass and auto-keep every gate-validated improvement (no per-visual approval):** work the Tier-3 loop per [dashboard-audit.md](resources/dashboard-audit.md): build the per-visual audit bundle (`audit_tier.build_dashboard_audit`, or `twb_to_pbir --audit`) → adjudicate each warned visual (choose a chart type **only** from that visual's listed `alternatives`; never drop, add, or re-role a field) → **run every proposal through `land_dashboard_audit`, which monotonic-gates each visual and keeps the assisted rebuild only when it regresses nothing** (match-or-beat). **You ARE the dashboard-audit compiler — there is no script that redesigns the visual for you; you adjudicate, the monotonic gate validates.** (The first-class on-disk `--approved-viz` re-emit seam — the viz twin of `--approved-dax` — lands in a later version; today the gate-validated result is produced in-process and the deterministic PBIR is the shipped baseline.) The vision/image oracle (next step) is the evidence this adjudication draws on.
   > _Ask, then run only on `GO` (do not proceed unprompted):_ "P of Q visuals rebuilt faithfully; R warned: `<Visual A>` on `<Sheet>`, … — run the LLM-assisted dashboard audit to attempt these? Reply `GO` to run it, or skip to ship the deterministic rebuild as-is."
6. **(Optional) Run the image oracle to settle ambiguous chart types** — for a workbook with non-standard / "hacky" views (a dual-axis pie that renders as a donut, a running-total Gantt that reads as a waterfall, an INDEX()/RANK() bump, a donut with a KPI floating in its hole), an opt-in **agent-driven vision pass** can confirm or correct each visual's *chart type* against the original Tableau rendering — **without ever touching field bindings**. It consumes the additive per-visual `candidate_records` `twb_to_pbir` already emits, resolves an offline-first image (caller-provided file → embedded `.twb`/`.twbx` thumbnail → none), and re-binds a visual's type **only** to a type in its candidate list. Follow the numbered runbook in [image-oracle.md](resources/image-oracle.md). Sheet swaps and field bindings stay deterministic; the Tier-1 report stands on its own if you skip this.
