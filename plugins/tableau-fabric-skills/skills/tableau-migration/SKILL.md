...
| [`scripts/work_order.py`](scripts/work_order.py) | **Tier-3 work order (per dashboard).** `--run <RUN> [--corpus <tc-corpus>]` → `<run>/out/work_orders/<Dashboard>.md`. Carries **only what the reference image cannot show**, and nothing else: calculations emitted as a placeholder `0` (with the Tableau source, the model columns they need, and **which visual is displaying the placeholder** — a `0` on a card is indistinguishable from a real measurement), values that render plausibly but are **wrong** (partial translation), and constructs **never rebuilt at all**. Colours, fonts, layout, sort order and styling are deliberately EXCLUDED — the downstream agent reads those off the image faster and more accurately than we can describe them. **It makes no claim that anything absent from it is correct**, and says so: the worklist records translation failures, not visual fidelity, and a visual that translated cleanly is usually the one still wearing the default theme. Measured: an "audit"-shaped work order that violated both rules made the same agent 2x SLOWER and produced a worse report than working from the image alone. |
