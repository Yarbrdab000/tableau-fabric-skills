...
| [`scripts/work_order.py`](scripts/work_order.py) | **Tier-3 work order (per dashboard).** Turns a finished run into the handoff document for whoever takes the report to full visual fidelity — `--run <RUN> [--corpus <tc-corpus>]` → `<run>/out/work_orders/<Dashboard>.md`. It **pushes pre-resolved answers instead of pointing at where to look**: each item carries the exact emitted `visual.json` path (joined on the worklist's recorded `visual` name — the same string as the visual's folder, so nothing is searched for and nothing is guessed), items are batched by **fix mechanism** so one procedure covers N visuals at the cost of one verify cycle, each batch states that procedure, and mechanism-relevant corpus precedent is inlined. `PART B` lists visuals nothing was flagged against so they are not re-audited — and is deliberately conservative, listing a visual only when it is provably unflagged **by name**; where an item could only be narrowed to a type, every visual of that type is withheld. **No time or step budget, stated explicitly** (a budget buys speed by shipping a worse report), and `PART D` says finishing the list is the START of the reader's judgement. |
