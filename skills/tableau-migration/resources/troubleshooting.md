...
| An estate size/complexity estimate says the big workbooks are trivial (0 calcs) | It was sized from the Metadata API's `embeddedDatasources`. A **published**-backed workbook keeps its calcs in the *datasource* — only a `sqlproxy` stub travels with it, so the hardest workbooks score as the easiest | Run **`estate_survey.py`** (STEP 0.5) before sizing. It resolves the `sqlproxy` edges from REST, flags every `complexity_understated` workbook, and emits a `fetch_order` that puts each datasource ahead of the workbooks needing it. |
| An estate dependency graph shows **no** dependencies at all | Dependencies were joined on the connection's `datasource.id`. That id is **not** the published datasource's site LUID, so the join matches nothing — and "nothing" reads as "no dependencies" rather than as an error | Join **by name** (`estate_survey.py` does). Because names can repeat across projects, a duplicate is reported `ambiguous` and never guessed — pick the project yourself. |
| Reference images (D7 / STEP 1.6) didn't appear, or a route failed | By design this **never** blocks a build — it always exits `0` and records the gap | Read `.\out\reference_images\manifest.json`: `summary.acquired` vs `summary.declared`, plus `missing` and `warnings`. The migration itself is unaffected. Re-run with a different `--mode`, or accept the gap. **Never gate STEP 2 on it.** |
| `--mode capture` won't run / can't find Tableau | The route needs Tableau Desktop on **this** machine, Windows PowerShell 5.1 with `-STA`, and the vendored `scripts/capture/` | Run `reference_images.py --mode capture --preflight --workbook <file>` — it names the blocker and changes nothing. If Tableau isn't installed, use `--mode export` with a Print-to-PDF or PowerPoint export instead. |
| An image is paired with the wrong dashboard | A PDF page is matched to a dashboard by **text content**; a dashboard that hides its chart titles offers little text to match on | Check `confidence` in the manifest — `named` (a PowerPoint's own per-slide sheet name) needs no inference at all, so **prefer a PowerPoint export** when the pairing matters. `content-weak` / `tab-order` are flagged `[VERIFY]` in the printed summary: confirm those by eye. |
| Reference images don't line up with the report pages | Something is pairing by **filename** | Pair on `manifest["images"][n]["dashboard"]` (the exact Tableau name) against the page's `displayName`. This script, the REST path and the PBIR emitter all sanitize names differently, and none can be derived from another. |
...
| Symptom | Likely cause | Do this now |
|---|---|---|
| Power BI Desktop: **`Method not found: 'Void Newtonsoft.Json.JsonSerializerSettings..ctor'`** on opening *any* `.pbip` — **including a brand-new blank one** | **Not our output.** An old `Newtonsoft.Json` is registered in the machine's **GAC** and Desktop binds to it. Nothing about the migration is involved | **The run already checks for this**: `report.json` → `environment.findings`, plus a loud `summary.md` banner. Check on demand with `py -3.11 scripts/environment_preflight.py`. Confirm independently by opening a **blank** `.pbip` — if that fails too, it is the machine. **Remedy, in an elevated PowerShell — no Windows SDK required:**<br>`Add-Type -AssemblyName System.EnterpriseServices`<br>`(New-Object System.EnterpriseServices.Internal.Publish).GacInstall('C:\Windows\Microsoft.NET\assembly\GAC_MSIL\Newtonsoft.Json\13.0.3__30ad4fe6b2a6aeed\Newtonsoft.Json.dll')`<br>then re-run the check. **Offer to run it; never run it unasked** — the GAC is machine-wide and this needs elevation, so it is the user's call. (`gacutil.exe -i <dll>` does the same thing but ships with the **Windows SDK**, which most analyst machines do not have — prefer the PowerShell form.) Background: [Fabric community thread](https://community.fabric.microsoft.com/t5/Desktop/Method-not-found-Void-Newtonsoft-Json-JsonSerializerSettings/m-p/5139648). **Reported by 2 of 3 users in one customer trial.** |
...
| I re-ran the migration and **hundreds of files differ** — is the engine non-deterministic? | Almost certainly not. A few terms differ between any two runs and carry no information: the **build root** embedded in each model's M `Source = Excel.Workbook(File.Contents("<root>\data\..."))` (so building to two directories changes every model file), `lineageTag` identity GUIDs, and timestamps | **Do not hand-roll the comparison** — five independent probes got this wrong in one day, variously reporting 85, 82 and "1 unexplained" phantom differences, including one that missed only the **JSON-escaped** spelling of the root. Run `py -3.11 scripts/build_diff.py <tree-a> <tree-b>`. It classifies each term and reports them **separately** — so `0 GUID differences` stays distinguishable from `GUIDs were masked` — and lists only what genuinely changed. Exit `1` means substantive differences, `2` means a bad path (never a clean run). Add `--strict` to treat identity-GUID churn as a failure too. |
| I compared against an **archived/copied baseline** and hundreds of **model** files differ | A build names the root it was **built at** inside every model's M `Source =` line. Copying or moving the tree does not rewrite that, so the embedded root no longer matches where the tree now lives | **Nothing is wrong with the build.** `build_diff.py` infers the build root from the artifacts themselves, so a relocated tree compares correctly with no extra flags — a byte-identical copy that previously read as **82 substantive differences** now reports **0**. If a tree's model layer was stripped, so there is nothing left to infer from, pass `--root-a` / `--root-b` with the path it was *built* at. The report prints the build roots it used for each tree, and warns when one tree's root matched nothing while the other's matched. |
| `powerbi-report-author validate` reports **`PBIR_SLICER_HEIGHT_BELOW_FLOOR`** on our output — *"Dropdown slicer height 57px < 76px minimum (header 28 + selector 32 + padding 8/8)"* | **A known false positive, and the only first-party rule this engine knowingly fails.** That rule computes the chrome of Power BI's **default (~12pt)** face. This emitter stamps the **authored** point size as `textSize` on the slicer's `header` and `items` wells (measured: **94 of 94** corpus slicers carry both, mostly `9D`), and the rule cannot see it — so it costs a card sized for a face we do not emit | **Do not "fix" it, and do not re-argue it from the validator.** The floor *was* 76.0, which is exactly that arithmetic; it was calibrated before the point-size stamp existed and then overrode the authored size on every dashboard — a 57px Tableau card silently grown to 76px, a third taller than the author drew it. Reverting this line has already shipped once (#180) and regressed **every** dropdown card in the corpus by 19px. The render is the authority over the rule: if the trade is ever revisited, **re-render** and judge it there. Expect roughly **11 of 68** corpus reports to carry this code; a report whose only errors are this one is a clean report. Rationale lives at `SLICER_DROPDOWN_MIN_H` in `scripts/twb_to_pbir.py`. |
