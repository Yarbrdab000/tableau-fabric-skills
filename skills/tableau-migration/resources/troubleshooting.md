...
| Symptom | Likely cause | Do this now |
|---|---|---|
| Power BI Desktop: **`Method not found: 'Void Newtonsoft.Json.JsonSerializerSettings..ctor'`** on opening *any* `.pbip` — **including a brand-new blank one** | **Not our output.** An old `Newtonsoft.Json` is registered in the machine's **GAC** and Desktop binds to it. Nothing about the migration is involved | **The run already checks for this**: `report.json` → `environment.findings`, and `summary.md` carries a loud banner when it fires. Check it on demand with `py -3.11 scripts/environment_preflight.py`. Prove it independently by creating a blank `.pbip` in Desktop and opening it — if that fails too, it is the environment. Fix by registering the current assembly: find `gacutil.exe` (e.g. `C:\Program Files (x86)\Microsoft SDKs\Windows\v10.0A\bin\NETFX 4.8 Tools\x64\`, part of the **Windows SDK** — it is not installed by default) and run `.\gacutil.exe -i C:\Windows\Microsoft.NET\assembly\GAC_MSIL\Newtonsoft.Json\13.0.3__30ad4fe6b2a6aeed\Newtonsoft.Json.dll` (expect *"Assembly successfully added to the cache"*). Background: [Fabric community thread](https://community.fabric.microsoft.com/t5/Desktop/Method-not-found-Void-Newtonsoft-Json-JsonSerializerSettings/m-p/5139648). **We detect this but never repair it** — the GAC is machine-wide and the fix needs elevation, so it is your call to make, not the migration's. **Reported by 2 of 3 users in one customer trial.** |
