...
MIN_SLICER = (120.0, 57.0)       # dropdown: MUST equal the emitter's SLICER_DROPDOWN_MIN_H (#180).
...
# It has not drifted -- 2.294.0 was 76/76, 2.295.0 moved both to 57/57, and 2.342.0/2.344.0 moved
# both again -- so the pin protects a property that holds rather than repairing one that broke.
# Worth having because the comment above states the cost precisely: emit re-floors a slicer after
# placement, so a reservation SMALLER than the emitter's floor does not shrink the emitted box, it
# makes it overrun whatever the solver seated below it.
