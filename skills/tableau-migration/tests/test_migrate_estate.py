...
<datasource formatted-name='Inventory Feed' inline='true' version='18.1'>
  <connection class='federated'>
    <named-connections>
      <named-connection caption='tdprod' name='teradata.bb22'>
        <connection class='teradata' dbname='INVENTORY' server='td.example.com' />
      </named-connection>
    </named-connections>
    <relation connection='teradata.bb22' name='Stock' table='[INV].[Stock]' type='table' />
    <metadata-records>
      <metadata-record class='column'>
...
def test_migrate_estate_local_full(fixtures_dir, tmp_path):
    ...
    # counts: one migrated SQL Server DS, one teradata fallback, one warned workbook
    ...
    assert s["connectors_seen"] == ["sqlserver", "teradata"]
